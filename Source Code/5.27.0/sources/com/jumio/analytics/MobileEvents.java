...
public class MobileEvents {
    ...
    public static final int EVENTTYPE_BENCHMARK = 314;
    ...
    public static AnalyticsEvent sdkLifecycle(UUID uuid, DismissType dismissType, @Nullable MetaInfo metaInfo) {
        return new AnalyticsEvent(EVENTTYPE_SDKLIFECYCLE, uuid, dismissType.toString(), metaInfo);
    }
    ...
    public static AnalyticsEvent mobileDeviceInformation(UUID uuid, String str, String str2, boolean z, boolean z2, boolean z3) {
        ...
        metaInfo.put("isTablet", Boolean.valueOf(z3));
    ...
    public static AnalyticsEvent networkRequest(UUID uuid, String str, int i, long j) {
        MetaInfo metaInfo = new MetaInfo();
        metaInfo.put("http", Integer.valueOf(i));
        metaInfo.put("roundtrip", Long.valueOf(j));
        return new AnalyticsEvent(EVENTTYPE_NETWORKCALL, uuid, str, metaInfo);
    }

    public static AnalyticsEvent benchmark(UUID uuid, double d) {
        MetaInfo metaInfo = new MetaInfo();
        if (Double.isNaN(d)) {
            d = 0.0d;
        }
        metaInfo.put("coremarkc", Double.valueOf(d));
        return new AnalyticsEvent(EVENTTYPE_BENCHMARK, uuid, "noValue", metaInfo);
    }

    public static AnalyticsEvent additionalData(UUID uuid, MetaInfo metaInfo) {
        return new AnalyticsEvent(EVENTTYPE_ADDITIONAL_DATAPOINTS, uuid, "noValue", metaInfo);
    }
}
