...
public class LivenessOverlay implements Overlay {
    ...
    public void addViews(@NonNull ViewGroup viewGroup) {
        ...
        this.descriptionTextView.setText(R.string.netverify_scanview_liveness_description);
    ...
    public static RelativeLayout getRotationLayout(Context context) {
        ...
        sVGImageView.setLayoutParams(new LayoutParams(dpToPx, dpToPx));
        ...
        ((TextView) relativeLayout.findViewById(R.id.helpview_doctype_title)).setText(Html.fromHtml(context.getString(R.string.netverify_scanview_liveness_landscape_header)));
        ((TextView) relativeLayout.findViewById(R.id.helpview_scan_instructions)).setText(R.string.netverify_scanview_liveness_landscape_description);
    ...
    public void doDraw(Canvas canvas) {
        if (this.visible) {
            ...
            path = this.borderPath;
            if (path != null) {
                paint = this.facePaint;
                if (paint != null) {
                    canvas.drawPath(path, paint);
                }
            }
            ...
            if (Log.isLogEnabledForLevel(LogLevel.DEBUG)) {
                RectF rectF = this.facePositionRect;
                if (rectF != null) {
                    paint = this.facePositionPaint;
                    if (paint != null) {
                        canvas.drawRect(rectF, paint);
                    }
                }
                rectF = this.validAlignmentRect;
                if (rectF != null) {
                    paint = this.validAlignmentPaint;
                    if (paint != null) {
                        canvas.drawRect(rectF, paint);
                    }
                }
    ...
    public void update(ExtractionUpdate extractionUpdate) {
        if (extractionUpdate.getState() == LivenessExtractionUpdateState.faceAlignmentOutOfBounds) {
            ...
            if (bundle.getBoolean(LivenessExtractionUpdateState.faceAlignmentInvalid_small, false)) {
                this.hintTextView.setText(R.string.netverify_scanview_liveness_move_closer);
                this.hintTextView.setVisibility(0);
            } else if (bundle.getBoolean(LivenessExtractionUpdateState.faceAlignmentInvalid_big, false)) {
                this.hintTextView.setText(R.string.netverify_scanview_liveness_move_back);
                this.hintTextView.setVisibility(0);
            } else {
                this.hintTextView.setVisibility(4);
            }
