...
public final class RetryAndFollowUpInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) {
        ...
        while (!this.canceled) {
            try {
                Response build;
                request = realInterceptorChain.proceed(request, r9, null, null);
                if (response != null) {
                    build = request.newBuilder().priorResponse(response.newBuilder().body(null).build()).build();
                }
                try {
                ...
                if (!recover(e2.getLastConnectException(), r9, false, request)) {
                    throw e2.getFirstConnectException();
                }
            } catch (IOException e3) {
                if (!recover(e3, r9, !(e3 instanceof ConnectionShutdownException), request)) {
                    throw e3;
                }
            } catch (Throwable th) {
    ...
    private Request followUpRequest(Response response, Route route) {
        if (response != null) {
            ...
            } else if (code == UnexpectedResponseException.STATUS_CODE_UNAUTHORIZED) {
    ...
    private int retryAfter(Response response, int i) {
        ...
        return header.matches("\\d+") ? Integer.valueOf(header).intValue() : Integer.MAX_VALUE;
    }
